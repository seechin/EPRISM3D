...
const char * szHelp2 = "\
    ...
    -debug[-level] 0/1/2    debug level, 0/1/2 = none/brief/details\n\
";
#ifdef _LIBZ_
...
const char * szHelpAdvanced = "\
  ...
  Advanced settings: (all optional)\n\
    ...
    -cr [on/off][,sf=1.5]   enable/disable cavity removal, default off\n\
    ...
    -errtol 1e-12           (-errtolrism, -errtolhi) error tolerance of SCF\n\
    ...
    -allow/-forbid          turn on/off options:\n\
        ...
        original-scf-error  allow/forbid output original SCF error, default off\n\
        [Coulomb-]renorm[alization] allow/forbid Coulomb renormalization in RISM\n\
        xvv                 allow/forbid to use xvv=wvv+nhkvv, default allowed\n\
...
const char * szHelpInternal = "\
  Internal features: (all optional, cautious!)\n\
    ...
    -pme-gamma              * long range PME decreasor, default value: 5/rcoul\n\
...
bool analysis_gvv_map(IET_Param * sys, char * argv[], int * argi, int argc, char * script_name, int script_line){
    ...
    if (nargs>=3){
        int atomi = -1; int atomj = -1; int icol = 0; int direction = 0; const char * failure_reason = nullptr;

        if (nargs==3 && StringNS::is_string_number(argv[2])){
            if (StringNS::is_string_number(argv[0])) atomi = atoi(argv[0]); else if (sys->atom_list) atomi = search_atom_list_grp(sys->atom_list, 0, sys->n_atom_list, argv[0], -1);
            if (StringNS::is_string_number(argv[1])) atomj = atoi(argv[1]); else if (sys->atom_list) atomj = search_atom_list_grp(sys->atom_list, 0, sys->n_atom_list, argv[1], -1);
            icol = atoi(argv[2]);
            direction = 0;
        } else failure_reason = "syntax error";

        if (icol<0) failure_reason = "incorrect column";

        if (failure_reason){
            fprintf(sys->log(), "%s%s : warning : ignoring gvv-map[%s-%s]=%d%s : %s\n", sys->is_log_tty?color_string_of_warning:"", software_name, argv[0], argv[1], icol, sys->is_log_tty?color_string_end:"", failure_reason);
        } else {
            if (atomi>=0 && atomj>=0){
                ...
                if (direction==0 && atomi>atomj){ int t = atomi; atomi = atomj; atomj = t; }
                sys->gvv_map[sys->n_gvv_map].grpi = atomi;
                sys->gvv_map[sys->n_gvv_map].grpj = atomj;
                sys->gvv_map[sys->n_gvv_map].col  = icol;
                sys->gvv_map[sys->n_gvv_map].direction = direction;
                sys->n_gvv_map ++;
            } else {
                fprintf(sys->log(), "%s%s : error : undefined", sys->is_log_tty?color_string_of_error:"", software_name);
                if (atomi<0) fprintf(sys->log(), " %s %s", StringNS::is_string_number(argv[0])?"atom group":"atom", argv[0]);
                if (atomi<0 && atomj<0) fprintf(sys->log(), " and");
                if (atomj<0) fprintf(sys->log(), " %s %s", StringNS::is_string_number(argv[1])?"atom group":"atom", argv[1]);
                fprintf(sys->log(), "%s\n", sys->is_log_tty?color_string_end:"");
                return false;
            }
        }
...
bool set_sys_options(IET_Param * sys, const char * sline, bool flag, const char * script_name, int script_line){
    ...
    for (int i=0; i<nw; i++){
        ...
        } else if (sl[i]=="original-scf-error" || sl[i]=="original-scf-err" || sl[i]=="original_scf_error" || sl[i]=="original_scf_err" || sl[i]=="original-error" || sl[i]=="original-err" || sl[i]=="original_error" || sl[i]=="original_err"){
            sys->err_from_unscaled_res = flag;
...
//for (int i=0; i<sys->n_rdf_grps; i++) printf("%d %d.%s.%s - %d.%s.%s = %d\n", i+1, sys->rdf_grps[i].is, sys->rdf_grps[i].ms, sys->rdf_grps[i].as, sys->rdf_grps[i].iv, sys->rdf_grps[i].mv, sys->rdf_grps[i].av, sys->rdf_grps[i].grp);
   ...
   // ---------------------------------------------------------------------------------
    ...
    } else if (key == "-cr" || key == "--cr" || key == "-cavity-remove" || key == "--cavity-remove" || key == "-cavity_remove" || key == "--cavity_remove" || key == "-cavity-removal" || key == "--cavity-removal" || key == "-cavity_removal" || key == "--cavity_remove_correction" || key == "-cavity-removal-correction" || key == "--cavity-removal-correction" || key == "-cavity_removal_correction" || key == "--cavity_removal"){
        ...
        if (i+1<argc && argv[i+1][0]!='-'){ i++; StringNS::string kl[20]; int nkl = StringNS::analysis_csv_line(argv[i], kl, 20, true, true);
            for (int k=0; k<nkl; k++){
                if (kl[k]=="on"){
                    ...
                    if (StringNS::is_string_number(&kl[k].text[7])) sys->cavity_removal_ucutoff = atof(&kl[k].text[7]);
                } else if (kl[k].length>=12 && (StringNS::string(kl[k].text,12) == "size-factor=" || StringNS::string(kl[k].text,5) == "size_factor=")){
                    if (StringNS::is_string_number(&kl[k].text[12])) sys->cavity_removal_size_factor = atof(&kl[k].text[12]);
                } else if (kl[k].length>=3 && (StringNS::string(kl[k].text,3) == "sf=")){
                    if (StringNS::is_string_number(&kl[k].text[3])) sys->cavity_removal_size_factor = atof(&kl[k].text[3]);
                } else {
                    fprintf(sys->log(), "%s : %s[%d] : invalid option for -cr : %s\n", software_name, get_second_fn(script_name), script_line, kl[k].text); ret = 1;
                }
            }
        }
        if (sys->cavity_removal_correction) sys->b_allow_r2uvmin = true;
   ...
   // ---------------------------------------------------------------------------------
        ...
        sys->debug_xvv = true; sys->display_xvv = false;
        if (i+1<argc && StringNS::is_string_number(argv[i+1])){
            i++; sys->debug_xvv_terms = atoi(argv[i]);
        }
    } else if (key=="-debug-display-xvv" || key=="debug-display-xvv" || key=="--debug-display-xvv" || key=="-debug_display_xvv" || key=="debug_display_xvv" || key=="--debug_display_xvv"){
        sys->debug_xvv = true; sys->display_xvv = true;
        if (i+1<argc && StringNS::is_string_number(argv[i+1])){
            i++; sys->debug_xvv_terms = atoi(argv[i]);
        }
...
int analysis_params_post(IET_Param * sys){
  ...
  // basic: parameter auto filling based on settings
    sys->nv = sys->nav;
    if (sys->gamma_auto_generate) sys->gamma_erf = 5 / (sys->rcoul<=0? 1e-12 : sys->rcoul);
  ...
  // analysis commands
    if (nw>0){
      ...
      // cmd: kernel calculation: HI, IET
            ...
            check_hi_diis_default_steps(sys);
        } else if (sl[0]=="ldi"){       cmd_type = 10; cmd.command = 2000+HIAL_LDI;       cmd.step = sys->stepmax_hi;
  ...
  // analysis parameters of commands
    if (success && nw>1){
        ...
        for (int i=1; i<nw; i++) if (sl[i].length>0){
            ...
            } else if (cmd_type==10){ // HI and LDI
